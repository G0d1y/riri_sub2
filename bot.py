...
def process_video_with_links(video_link, subtitle_link, client, chat_id, output_name):
    ...
    if cancel_event.is_set() == False:
        ...
        trimmed_url = f"https://t.me/{chat_id}/{trimmed.id}"
        ...
        trimmed_low_url = f"https://t.me/{chat_id}/{trimmed_low.id}"
        ...
        final_url = f"https://t.me/{chat_id}/{final.id}"
...
async def handle_cover(client, message):
    try:
        ...
        await message.send_image("-1002310252740" , message.file_id)
        await message.send_image("-1002332192205" , message.file_id)
